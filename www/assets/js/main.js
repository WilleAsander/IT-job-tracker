...
function getAmmount(userRegion) {
    ...
    $.ajax({
        ...
        url: 'http://api.arbetsformedlingen.se/af/v0/platsannonser/matchning?nyckelord=stockholm&yrkesomradeid=3',
...
function getJobDetails(annonsId) {
    $.ajax({
        ...
        success: function (result) {
            ...
            jobDesc = result.platsannons.annons.annonstext;
            ...
            console.log(result);
            initialize(address, jobName, jobPlats,jobType,jobLenght,jobRegisterday,jobWage,linkID,link,jobDesc);
...
function initialize(address, jobName,jobPlats,jobType, jobLenght, jobRegisterday, jobWage,link, linkID, jobDesc) {
    ...
    geocoder.geocode({ 'address': address }, function (results, status) {
        if (status == google.maps.GeocoderStatus.OK) {
            ...
            if (distance_from_location <= 14000) {
            ...
            google.maps.event.addListener(marker, 'click', (function (marker) {
                return function () {
                    ...
                    infowindow.setContent(
                        '<div id =hello>'+
                        '<h1>' + jobName + '</h1>' + 
                        '<p> Jobbadress: ' + address +  
                        '<p> Beskrivning: <br>' + jobDesc +
                        '<p> Anställningsform: ' + jobType +
                        '<p> Varaktighet: ' + jobLenght +
                        '<p> Lön: ' + jobWage + 
                        '<p><a target="_blank" rel="noopener noreferrer" href="https://www.arbetsformedlingen.se/For-arbetssokande/Hitta-jobb/Platsbanken/annonser/'+link+'">Annonslänk</a>');
                        + '</p>' + '</div>'
